...
# ─────────────────────────────────────────
# WIN32 API (console hide — called later)
# ─────────────────────────────────────────
...
"@
# Console is hidden only right before the GUI launches (see bottom of script).
# Hiding it here would suppress any startup errors silently.
...
$AllTweaks = @(
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Verhindert automatische Windows Update Neustarts während du spielst. Setzt Active Hours auf 0–23 Uhr und deaktiviert Auto-Reboot mit eingeloggtem Nutzer."
        Category = "Gaming"
        Group    = "In-Game Boosts"
        Action   = {
            reg add "HKLM\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings" /v ActiveHoursStart /t REG_DWORD /d 0 /f | Out-Null
            ...
            Write-Log "Windows Update disabled during gaming"
        }
    },
    [PSCustomObject]@{
        Name     = "Disable Background App Throttling"
        Desc     = "Deaktiviert das Windows-Throttling für Hintergrund-Apps. Verhindert dass Windows Ressourcen für Hintergrundprozesse künstlich begrenzt — mehr Performance für dein Spiel."
        Category = "Gaming"
        Group    = "In-Game Boosts"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Aktiviert DirectX 12 Ultimate Optimierungen via Registry und SwapEffect-Upgrade. Verbessert Frame-Pacing und reduziert Latenzen in DX12-Spielen."
        Category = "Gaming"
        Group    = "GPU & Driver"
        Action   = {
            if (-not (Test-Path "HKLM:\SOFTWARE\Microsoft\DirectX\UserGpuPreferences")) {
                New-Item -Path "HKLM:\SOFTWARE\Microsoft\DirectX\UserGpuPreferences" -Force | Out-Null
            }
            reg add "HKLM\SOFTWARE\Microsoft\DirectX\UserGpuPreferences" /v DirectXUserGlobalSettings /t REG_SZ /d "SwapEffectUpgradeEnable=1;" /f | Out-Null
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Deaktiviert den Windows Network Throttling Index (auf 0xFFFFFFFF gesetzt = unbegrenzt). Windows drosselt sonst Netzwerkpakete bei hoher CPU-Last — besonders schädlich für Online-Spiele."
        Category = "Network"
        Group    = "Latency"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Setzt den DNS-Server auf Google DNS 8.8.8.8 (Primary) und 8.8.4.4 (Secondary). Zuverlässig, schnell und weltweit verfügbar. Alternative zu Cloudflare DNS."
        Category = "Network"
        Group    = "DNS"
        Action   = {
            $adapters = Get-NetAdapter | Where-Object { $_.Status -eq "Up" }
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Aktiviert ECN (Explicit Congestion Notification), SACK (Selective Acknowledgment) und deaktiviert TCP Timestamps. Verbessert Throughput, reduziert Paketverluste und Overhead."
        Category = "Network"
        Group    = "TCP"
        Action   = {
            ...
            Write-Log "TCP Settings optimized (ECN on, SACK on, Timestamps off)"
        }
    },
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Setzt die Auslagerungsdatei auf 'Vom System verwaltet'. Windows passt dann die Größe dynamisch an den Bedarf an — optimal für die meisten Systeme."
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Löscht die Auslagerungsdatei beim Herunterfahren. Verhindert dass sensible Daten im PageFile verbleiben. Verlängert die Shutdown-Zeit leicht."
        Category = "RAM & Storage"
        Group    = "Memory"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Deaktiviert die Windows RAM-Komprimierung (Memory Compression). Kann CPU-Overhead reduzieren auf Systemen mit ausreichend RAM (16 GB+). Nicht empfohlen bei wenig RAM."
        Category = "RAM & Storage"
        Group    = "Memory"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Aktiviert TRIM für SSDs (fsutil DisableDeleteNotify=0). TRIM teilt der SSD mit welche Blöcke frei sind — hält die SSD-Performance langfristig hoch."
        Category = "RAM & Storage"
        Group    = "SSD & NVMe"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Deaktiviert die automatische geplante Defragmentierung. Auf SSDs ist Defragmentierung unnötig und erhöht die Schreiblast. Auf HDDs kann sie weiterhin manuell ausgeführt werden."
        Category = "RAM & Storage"
        Group    = "SSD & NVMe"
        Action   = {
            schtasks /Change /TN "\Microsoft\Windows\Defrag\ScheduledDefrag" /Disable 2>$null
            Write-Log "Scheduled Defrag disabled"
        }
    },
    [PSCustomObject]@{
        Name     = "Optimize NVMe Queue Depth"
        Desc     = "Optimiert die NVMe-Warteschlangentiefe über den stornvme-Treiber. Verbessert den I/O-Durchsatz bei modernen NVMe SSDs mit hoher Queue-Tiefe."
        Category = "RAM & Storage"
        Group    = "SSD & NVMe"
        Action   = {
            ...
            Set-ItemProperty -Path $nvmePath -Name "TreatAsInternalPort" -Value ([byte[]](0,0,0,0,0,0)) -Type Binary -ErrorAction SilentlyContinue
            Write-Log "NVMe Queue Depth optimized"
        }
    },
    [PSCustomObject]@{
        Name     = "Disable Write-Cache Buffer Flushing"
        Desc     = "Deaktiviert das erzwungene Leeren des Schreib-Cache-Puffers. Verbessert die Schreibleistung von Festplatten und SSDs — leichtes Datenverlustrisiko bei unerwartetem Stromverlust."
        Category = "RAM & Storage"
        Group    = "SSD & NVMe"
        Action   = {
            $disks = Get-WmiObject -Class Win32_DiskDrive
            foreach ($disk in $disks) {
                $pnpId = $disk.PNPDeviceID -replace "\\", "\\"
                $regPath = "HKLM:\SYSTEM\CurrentControlSet\Enum\$($disk.PNPDeviceID)\Device Parameters\Disk"
                if (Test-Path $regPath) {
                    Set-ItemProperty -Path $regPath -Name "UserWriteCacheSetting" -Value 1 -Type DWord -ErrorAction SilentlyContinue
                }
            }
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Deaktiviert den Ruhezustand (Hibernation) und löscht hiberfil.sys. Gibt je nach RAM-Größe 4–32 GB Speicherplatz frei. Schnellstart (Fast Boot) bleibt davon unberührt."
        Category = "RAM & Storage"
        Group    = "Cleanup"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Löscht alle Dateien in %TEMP% und C:\Windows\Temp. Gibt Speicherplatz frei und kann Systemstart leicht beschleunigen. Laufende Prozesse werden dabei nicht beeinträchtigt."
        Category = "RAM & Storage"
        Group    = "Cleanup"
        Action   = {
...
function New-TweakRow {
    ...
    # Info button — uses XAML InfoBtn style (hover handled by ControlTemplate.Triggers)
...
$BtnApply.Add_Click({
    ...
    try {
        Checkpoint-Computer -Description "GameOptimizerPro v3.0 Backup" -RestorePointType "MODIFY_SETTINGS" -ErrorAction Stop
...
# ─────────────────────────────────────────
# LAUNCH — hide console only now, right before GUI appears
# ─────────────────────────────────────────
Write-Log "GameOptimizerPro v3.0 started | $HWInfo"
try {
    [WinConsole]::ShowWindow([WinConsole]::GetConsoleWindow(), 0) | Out-Null
    $Window.ShowDialog() | Out-Null
} catch {
    [System.Windows.MessageBox]::Show(
        "Startup error:`n`n$_`n`nCheck log: $LogFile",
