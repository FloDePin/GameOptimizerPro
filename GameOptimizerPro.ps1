...
$HasNVMe    = ($null -ne (Get-WmiObject -Query "SELECT * FROM Win32_DiskDrive" | Where-Object { $_.Model -match "NVMe|NVME" } | Select-Object -First 1))
...
$AllTweaks = @(
    ...
    [PSCustomObject]@{
        ...
        Desc     = "NEU v3.0: Verhindert dass Windows Update waehrend einer Gaming-Session aktiv wird oder das System belastet. Windows Update-Dienst wird pausiert und erst nach dem Gaming wieder aktiviert. Empfohlen fuer competitive Gaming."
        Category = "Gaming"
        Group    = "In-Game Boosts"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Action   = {
            try {
                ...
                Write-Log "Memory Compression: $_ (may require reboot or already disabled)"
            }
        }
    },
        ...
        Action   = {
            ...
            if ($nvmeDisks) {
                foreach ($disk in $nvmeDisks) {
                    ...
                    $regPath = "HKLM\SYSTEM\CurrentControlSet\Enum\$pnpId\Device Parameters\StorPort"
                    reg add $regPath /v QueueDepth /t REG_DWORD /d 32 /f | Out-Null
                }
    ...
    [PSCustomObject]@{
        ...
        Group    = "SSD & NVMe"
        Action   = {
            $disks = Get-WmiObject -Query "SELECT * FROM Win32_DiskDrive WHERE MediaType='Fixed hard disk media'"
