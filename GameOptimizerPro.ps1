...
$AllTweaks = @(
    ...
    [PSCustomObject]@{
        ...
        Action   = {
            ...
            $line = (powercfg -list | Select-String "Ultimative Leistung|Ultimate Performance" | Select-Object -First 1).ToString()
            $guid = if ($line -match '([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})') { $Matches[1] } else { $null }
