...
# ── STA-GUARD ─────────────────────────────────────────────────────────────────
# WPF (ShowDialog / MessageBox) REQUIRES Single-Threaded Apartment (STA).
# If PowerShell was started without -STA the window will silently fail to open.
# This block detects MTA mode and restarts itself correctly.
if ([System.Threading.Thread]::CurrentThread.ApartmentState -ne
    [System.Threading.ApartmentState]::STA) {
    Start-Process powershell.exe `
        -ArgumentList "-STA -ExecutionPolicy Bypass -NoProfile -File `"$PSCommandPath`"" `
        -Verb RunAs
    exit
}
# ─────────────────────────────────────────────────────────────────────────────
...
$Window.ShowDialog() | Out-Null
    ...
    # Show error in MessageBox instead of silent crash
    $errMsg = $_.Exception.Message
    $errLine = $_.InvocationInfo.ScriptLineNumber
    try {
        Add-Type -AssemblyName System.Windows.Forms -ErrorAction SilentlyContinue
        [System.Windows.Forms.MessageBox]::Show(
            "GameOptimizerPro failed to start.`n`nError (Line $errLine):`n$errMsg`n`nLog: $env:TEMP\GameOptimizerPro_Error.txt",
            "GameOptimizerPro - Startup Error",
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Error
        )
    } catch { }
