...
# Console is hidden IMMEDIATELY at startup so no PowerShell window lingers,
# even when launched via 'irm | iex' in an existing terminal.
# -----------------------------------------
...
# Hide now (nCmdShow = 0 => SW_HIDE)
try {
    $consoleHwndEarly = [Native.Win32Console]::GetConsoleWindow()
    if ($consoleHwndEarly -ne [IntPtr]::Zero) {
        [Native.Win32Console]::ShowWindow($consoleHwndEarly, 0) | Out-Null
    }
} catch { }
