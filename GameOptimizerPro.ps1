...
$CheckFunctions = @{
    ...
    # PRIVACY
    "Disable Telemetry & Data Collection" = { (Get-Service DiagTrack -EA SilentlyContinue)?.StartType -eq "Disabled" -or ((Get-RegVal "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection" "AllowTelemetry") -eq 0) }
    ...
    "Disable Prefetch & Superfetch"      = { (Get-Service SysMain -EA SilentlyContinue)?.StartType -eq "Disabled" -or (Get-Service SysMain -EA SilentlyContinue)?.Status -eq "Stopped" }
    ...
    "Disable Windows Search Indexing"    = { (Get-Service WSearch -EA SilentlyContinue)?.StartType -eq "Disabled" }
    ...
    # RAM & STORAGE
    "Optimize PageFile (System Managed)" = { (Get-WmiObject Win32_ComputerSystem -EA SilentlyContinue)?.AutomaticManagedPagefile -eq $true }
    ...
    "Disable Audio Enhancements"         = {
        ...
        (Get-ItemProperty $fx -Name "{1da5d803-d492-4edd-8c23-e0c0ffee7f0e},5" -EA SilentlyContinue)?. "{1da5d803-d492-4edd-8c23-e0c0ffee7f0e},5" -eq 1
    }
    ...
    "AMD: Disable ULPS (Ultra Low Power State)" = {
        ...
        (Get-ItemProperty $first.PSPath -Name "EnableULPS" -EA SilentlyContinue)?.EnableULPS -eq 0
    }
    ...
    "AMD: Anti-Lag (Low Latency Mode)"   = {
        ...
        $first = Get-ChildItem $ac -EA SilentlyContinue | Select-Object -First 1
        if (-not $first) { return $null }
        (Get-ItemProperty $first.PSPath -Name "EnableAntiLag" -EA SilentlyContinue)?.EnableAntiLag -eq 1
    }
