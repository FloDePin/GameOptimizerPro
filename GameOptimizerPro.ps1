...
function Get-UIString {
    ...
    if ($Script:UIStrings.ContainsKey($Id)) {
        $entry = $Script:UIStrings[$Id]
        if ($entry.ContainsKey($lang)) { return $entry[$lang] }
        return $entry["EN"]
    }
    return $Id
}
...
$BtnLang.Add_Click({
    if ($LangState.Current -eq "EN") {
        ...
        Apply-Language
        $StatusText.Text     = Get-UIString "status_lang_de"
    } else {
        ...
        Apply-Language
        $StatusText.Text     = Get-UIString "status_lang_en"
    }
})
