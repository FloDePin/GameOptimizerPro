    ...
    [PSCustomObject]@{
        ...
        Action   = {
            ...
            if (-not $disks) {
                ...
                Write-Log ("Write-Cache Buffer Flushing disabled ({0} disk(s) updated)" -f $count)
            }
        }
    },
...
$TweakDescEN = @{
    ...
    "Disable Windows Update during Gaming"        = "NEW v3.0: Permanently disables Windows Update auto-download via registry. Windows Update won't interrupt or background-load during gaming."
    "Disable Background App Throttling"           = "NEW v3.0: Disables Windows CPU throttling for background processes. Prevents Windows from secretly reducing game CPU time when background tasks are active."
    ...
    "Enable DirectX 12 Optimization"             = "NEW v3.0: Optimizes DirectX 12 settings for maximum gaming performance. Enables DX12 multi-threading and reduces draw call overhead."
    ...
    "Disable Network Throttling Index"            = "NEW v3.0: Disables Windows network packet throttling under high CPU load. Gives the network stack the highest priority."
    ...
    "Set DNS to Google (8.8.8.8)"                = "NEW v3.0: Sets DNS to Google 8.8.8.8 (Primary) and 8.8.4.4 (Secondary). Google's globally distributed, fast and reliable DNS. Alternative to Cloudflare."
    ...
    "Optimize TCP Settings (ECN/SACK/Timestamps)" = "NEW v3.0: Optimizes advanced TCP settings: disables ECN, enables SACK, disables TCP Timestamps. Reduces overhead and improves stability in online games."
    ...
    "Optimize PageFile (System Managed)"          = "NEW v3.0: Sets the pagefile to system managed. Windows dynamically adjusts it to RAM needs — prevents both too-small and too-large pagefiles."
    "Clear PageFile on Shutdown"                  = "NEW v3.0: Clears the pagefile on every shutdown. Prevents sensitive data from remaining on disk after reboot. Good for privacy."
    "Disable Memory Compression"                  = "NEW v3.0: Disables RAM compression in Windows. Saves CPU cycles during gaming. Recommended when you have enough RAM (16GB+)."
    "Enable SSD TRIM"                             = "NEW v3.0: Enables TRIM for all connected SSDs. Informs the SSD about unused blocks — maintains SSD performance long-term and extends lifespan."
    "Disable Scheduled Defragmentation"           = "NEW v3.0: Disables automatic scheduled defragmentation. Absolutely not recommended for SSDs — this tweak ensures it's turned off."
    "Optimize NVMe Queue Depth"                   = "NEW v3.0: Optimizes queue depth for NVMe drives. More parallel I/O operations noticeably improve NVMe SSD read/write performance."
    "Disable Write-Cache Buffer Flushing"         = "NEW v3.0: Disables forced write-cache buffer flushing for SSDs. Noticeably improves write speed. Recommended for desktops with stable power supply."
    "Disable Hibernation"                         = "NEW v3.0: Disables hibernate mode and removes hiberfil.sys. Frees several GB of disk space (equals your RAM amount). Recommended for desktop PCs."
    "Clean Temp Files"                            = "NEW v3.0: Deletes all files in %TEMP%, Windows\Temp and Prefetch folders. Frees disk space and can slightly speed up boot."
