...
# -----------------------------------------
# Add-Type mit try/catch — robuster als PSTypeName-Guard (der bei Typ-Konflikten fehlschlagen kann)
try {
...
"@ -ErrorAction Stop
    ...
    # Typ existiert bereits aus vorheriger Session — das ist OK, weiter machen
}
...
function New-TweakRow {
    ...
    try { $btn.Style = $Window.FindResource("InfoBtn") } catch {
        # Style nicht gefunden - Button bleibt mit Default-Style
