...
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    [System.Windows.MessageBox]::Show("Please run this script as Administrator!", "WinTweaker - Admin Required", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error)
    exit
}
...
$AllTweaks = @(
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Entfernt Microsoft Teams (Consumer-Version). Nicht zu verwechseln mit Teams for Work. Blockiert automatische Neuinstallation."
        Category = "Windows"
        Group    = "Bloatware"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Deaktiviert die Werbe-ID die Windows jedem Nutzer zuweist. Apps koennen dich dann nicht mehr geraeteuebergreifend tracken."
        Category = "Windows"
        Group    = "Privacy"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Deaktiviert den Windows Search Indexer (WSearch). Reduziert staendige Festplattenzugriffe im Hintergrund. Suche im Explorer funktioniert weiterhin, aber langsamer ohne Index."
        Category = "Windows"
        Group    = "Performance"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Deaktiviert Windows Fullscreen Optimizations global. Manche Spiele laufen im 'Borderless Windowed' statt echtem Fullscreen - dieser Tweak erzwingt echtes Fullscreen fuer niedrigeren Input-Lag."
        Category = "Gaming"
        Group    = "In-Game Boosts"
        Action   = {
    ...
    [PSCustomObject]@{
        Name     = "Enable HAGS"
        Desc     = "Aktiviert Hardware-Accelerated GPU Scheduling (HAGS). Windows uebergibt GPU-Scheduling direkt an die Hardware statt Software. Reduziert CPU-Overhead und leicht den Input-Lag. Erfordert NVIDIA RTX 2000+ oder AMD RX 5000+ und Windows 10 2004+."
        Category = "Gaming"
        Group    = "GPU and Driver"
        Action   = {
    ...
    [PSCustomObject]@{
        ...
        Desc     = "Deaktiviert die automatische TCP-Empfangsfenstergroesse. Kann auf manchen Systemen Latenz-Spikes reduzieren. Bei Highspeed-Internet (1 Gbit+) kann dies den Durchsatz leicht verringern."
        Category = "Network"
        Group    = "TCP"
        Action   = {
    ...
    <Window.Resources>
        ...
        <Style TargetType="Button" x:Key="InfoBtn">
            ...
            <Setter Property="Template">
                <Setter.Value>
                    <ControlTemplate TargetType="Button">
                        <Border x:Name="brd" Background="#16213e" CornerRadius="11"
                        ...
                        <ControlTemplate.Triggers>
                            <Trigger Property="IsMouseOver" Value="True">
                                <Setter TargetName="brd" Property="Background" Value="#e94560"/>
                                <Setter TargetName="brd" Property="BorderBrush" Value="#e94560"/>
                ...
                <Setter.Value>
                    <ControlTemplate TargetType="TabItem">
                        <Border Name="Border" Background="{TemplateBinding Background}"
                                CornerRadius="6,6,0,0" Margin="2,0" Padding="{TemplateBinding Padding}">
...
function New-TweakRow {
    ...
    # Info button — uses InfoBtn XAML style, hover handled by ControlTemplate.Triggers
    $btn         = New-Object Windows.Controls.Button
    $btn.Style   = $Window.Resources["InfoBtn"]
    $btn.ToolTip = $Tweak.Desc
